...
def upload_to_gist(nodes_by_group, stats, gist_id=None):
    ...
    # Base64 编码订阅
    all_content = "\n".join(all_links)
    subscription_b64 = base64.b64encode(all_content.encode('utf-8')).decode('utf-8')
    ...
    gist_data = {
        ...
        "public": False,
        "files": {
            "subscription.txt": {"content": subscription_b64},
...
def main():
    ...
    # 读取已有的 Gist ID
    global GIST_ID
    if not GIST_ID and os.path.exists("gist_id.txt"):
        with open("gist_id.txt", "r") as f:
            GIST_ID = f.read().strip()
        print(f"📝 使用现有 Gist: {GIST_ID}")

    new_gist_id, sub_url = upload_to_gist(nodes_by_group, stats, GIST_ID)

    # 清理临时文件
    cleanup_temp_dir(temp_dir)

    if new_gist_id:
        print(f"🆕 新 Gist ID: {new_gist_id}")
    else:
        print("❌ 同步失败")
